...
average of all elements in nums between the indices i - k and i + k (inclusive). If there are less
than k elements before or after the index i, then the k-radius average is -1.
...
int main() {
    ...
    int k1 = 3;
    res = sol.getAverages(nums1, k1);
    printArray(res);

    vector<int> nums2 = {100000};
    int k2 = 0;
    res = sol.getAverages(nums2, k2);
    printArray(res);

    vector<int> nums3 = {8};
    int k3 = 100000;
    res = sol.getAverages(nums3, k3);
    printArray(res);
